...
int raStorage::storeData( int priority, int data_length, unsigned char * input_data )
{
  ...
  if ( rc != 0 ) {
    this->logger->log( "%s not stored len=%d prio=%d, err=%d",
                       this->identity,
                       data_length, priority, rc );
    return rc;
  }
  ...
  if ( this->capacity < (this->end_mark + data_length + RA_STORAGE_HEADER_SIZE + RA_STORAGE_HEADER_SIZE ) ) {
    ...
    if ( space_freed < need_bytes ) {
      ...
      this->logger->log( "%s not stored, no space left len=%d prio=%d ",
                         this->identity,
  ...
  this->logger->log( "%s stored len=%d prio=%d at pos %d, usage %d/%d B",
                     this->identity,
                     data_length, priority, this->end_mark,
                     (this->end_mark+data_length+2), this->capacity );
...
{
  this->storeData( priority, strlen(input_string) + 1, (unsigned char*)input_string );
}
...
void raStorage::purgeDeleted()
{
  ...
  this->logger->log( "%s purge, saved %d B, usage %d/%d B", this->identity, (prev_end_mark - this->end_mark), this->end_mark, this->capacity );
...
void raStorageRecord::markAsDeleted()
{
  ...
  this->logger->log( "%s deleted, pos %d, prio %d", this->identity, (this->record_ptr - this->data_start), this->priority );
}

void raStorageRecord::markAsNotDeleted()
{
  this->record_ptr[0] &= 0x7F;
  this->logger->log( "%s undeleted, pos %d, prio %d", this->identity, (this->record_ptr - this->data_start), this->priority );
}
